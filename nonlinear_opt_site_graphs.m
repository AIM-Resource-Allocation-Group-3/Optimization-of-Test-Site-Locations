...
function[x,fval,exitFlag,Output] = nonlinear_opt_site_graphs(currentI,didt,neib,dneib,possTest,dist,fixneib,fixdneib,possneib,possdneib)
...
Aeq = [];
beq = [];
A = ones(1,length(possTest));
b = C;
lb = zeros(length(possTest),1);
ub = ones(length(possTest),1);
% ub(possTest) = 1;
nonlcon = [];
intcon = 1:length(possTest);
initPop = zeros(100,length(possTest));
...
[x,fval,exitFlag,Output] = ga(fun,length(possTest),A,b,Aeq,beq,lb,ub,nonlcon,intcon,options);
end
...
%             - G(2)*sum(F(nnode)) - M(3)*sum(R(dnode)) - G(3)*sum(F(dnode)));
        L(node) = ((didt(nnode)'*I(nnode))/dist + (didt(dnode)'*I(dnode))/(2*dist))*...
            (1 - M(1)*0 - G(1)*F(node) - M(2)*length(fixn) - G(2)*length(possn) - M(3)*length(fixd) - G(3)*length(possd));
    end
    L = -F*L;
end